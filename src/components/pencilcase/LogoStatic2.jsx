...
const Logo = () => {
  ...
  useEffect(() => {
    if (!toggle && active && loaded < 25) {
      setToggle(true)
    }
    if (toggle && loaded >= 25 && !active) {
      setToggle(false)
      setTimeout(() => {
        setVisible('none')
      }, 1000)
  ...
  return transition(() => (
    <a.div className={styles.logoBackgroundStatic} style={{ opacity: props.opacity, display: visible }}>
      <img
        src='https://res.cloudinary.com/drixmykpt/image/upload/v1660215790/hype/images/peaek5fuhka1zby4priz.png'
        alt=''
        width={350}
        className={styles.logoInnerStatic}
      />
      <InnerSVG />
      <OutlineSVG />
      ...
      <div className={styles.loadingFontContainer}>
        ...
        <div className={styles.loadingBarContainer}>
          <a.div className={styles.loadingBar} style={{ width: `${(loaded / 25) * 200}px` }}></a.div>
        </div>
      </div>
    </a.div>
  ))
